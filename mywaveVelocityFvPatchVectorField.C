...
#include "mywaveVelocityFvPatchVectorField.H"
#include "volFields.H"
...
Foam::
mywaveVelocityFvPatchVectorField::
mywaveVelocityFvPatchVectorField
(
    const fvPatch& p,
...
mywaveVelocityFvPatchVectorField
(
    const mywaveVelocityFvPatchVectorField& ptf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
:
    ...
    uWind_(dict.lookupOrDefault("uWind",vector(0.,0.,0.))),
...
void Foam::mywaveVelocityFvPatchVectorField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    if (!allCheck_ || reread)
    {
        ...
        uWind_ = (waveDict.lookupOrDefault<vector>("uWind",vector(0,0,0)));
    ...
    // Force ASCII writing
    waveDict.writeObject
    (
        IOstream::ASCII,           // Force ASCII format
        IOstream::currentVersion,  // Use current version
        IOstream::UNCOMPRESSED,    // No compression
        true                       // Write regardless of status
    );
        }
    } // End of allCheck
...
//    inlinePrint( "Measured Level ", measuredLevels );
    ...
    operator==
        ...
        *timeMult+uWind_*(1.0-pos(alphaCell-0.5)));

    fixedValueFvPatchField<vector>::updateCoeffs();
}


void Foam::mywaveVelocityFvPatchVectorField::write(Ostream& os) const
{
    ...
    os.writeKeyword("waveDictName") << waveDictName_ << token::END_STATEMENT << nl;
        ...
        os.writeEntryIfDifferent<vector>("uWind", vector::zero, uWind_);
        ...
        writeEntryIfDifferent<vector>(os, "uWind", vector::zero, uWind_);
